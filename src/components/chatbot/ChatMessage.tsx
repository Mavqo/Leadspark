...
/**
 * Formats a timestamp into Italian locale time string
 * @param timestamp - Date to format
 * @returns Formatted time string (HH:mm) or empty string if no timestamp
 */
 ...
 * Optimized with React.memo to prevent unnecessary re-renders in message lists
...
export const ChatMessage: React.FC<ChatMessageProps> = memo(({ message }) => {
  ...
  const senderLabel = isBot ? "Assistente" : "Tu";
  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col max-w-[80%] ${isBot ? 'items-start' : 'items-end'}`}>
        {/* Sender label - visually hidden for screen readers */}
